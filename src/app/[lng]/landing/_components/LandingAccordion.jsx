      ...
      {items.map((item, index) => (
        <div key={index} className="mb-2 rounded-lg">
          <button onClick={() => toggleItem(index)} className="w-full"> 
            <div className="cursor-pointer flex items-center justify-between bg-[#2D2D2D] transition-all duration-300 hover:bg-[#3d3d3d] h-[74px] px-[24px]">
              <p className="font-roboto font-normal text-white text-[24px] leading-[28.13px]">
                {item.title}
              </p>
              <Image
                src={accordionPlus}
          ...
          >
            <div className="px-4 py-2 bg-[#2D2D2D]">
              {item.content.split("[break]").map((paragraph, idx) => (
                <p
                  key={idx}
                  className="mb-2 p-4 text-white font-roboto font-normal text-[24px] leading-[28.13px]"
                >
                  {paragraph}
                </p>
              ))}
